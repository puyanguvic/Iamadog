...
std::string dir = "results/Exp2/OSPF/";
std::string ExpName = "OspfExperiment2";
...
// Function to install Send application with budget
...
// Function to install BESend application without budget
...
main (int argc, char *argv[])
{
  ...
  // Simulator::ScheduleNow (&CheckQueueSize,  grid.GetNetDevice-> GetQueueDiscClass(0), qID1f);
  ...
  // ------------------------------------*** OSPF Traffic Target Flow (BottomLeft --> TopRight)-------------------------------------  
  ...
  for (int i=1; i<11; i++)
  {
    InstallBEPacketSend (grid.GetNode (0, 0), sinkAddress, i-1, i, PacketSize, NPacket, i*10, true);
  }
