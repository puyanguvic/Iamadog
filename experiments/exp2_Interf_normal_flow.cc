...
main (int argc, char *argv[])
{
  ...
  tch.AddChildQueueDisc (handle, cid[2], "ns3::FifoQueueDisc", "MaxSize", StringValue("120p"));
  ...
  for (int i=0; i < 13; i ++)
  {
    ...
    interface[i].SetMetric (0, delayInMicro1 + 209);
    interface[i].SetMetric (1, delayInMicro1 + 209);
  }
  ...
  // InstallBEPacketSend (nodes.Get (0), sinkAddress, 0.0, 10.0, Packetsize, NPacket, "5Mbps", true);
  ...
  for (int i=0; i < 10; i ++)
  {
    ...
    std::ostringstream subset;
    subset<<i+1<<"Mbps";
    InstallBEPacketSend (nodes.Get (9), sinkAddress_1, startTime, endTime, Packetsize1, NPacket1, subset.str().c_str(), false);
  }
  
  // create a dsrSender application

  //uint32_t budget_1 = 20;
  //InstallDGPacketSend (nodes.Get (9), sinkAddress_1, 0.0, 2.0, Packetsize1, NPacket1, budget_1, "1Mbps", false);
