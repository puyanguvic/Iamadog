...
main (int argc, char *argv[])
{
  ...
  // for (int i = 0;i < 12; i ++)
  // {
  //   csma[i].SetDeviceAttribute ("DataRate", DataRateValue (DataRate (channelDataRate1)));
  //   csma[i].SetChannelAttribute ("Delay", TimeValue (MicroSeconds (delayInMicro1)));
  // }

  // {
  //   csma[12].SetDeviceAttribute ("DataRate", DataRateValue (DataRate (channelDataRate2)));
  //   csma[12].SetChannelAttribute ("Delay", TimeValue (MicroSeconds (delayInMicro2)));
  // }
  ...
  for (int i = 0; i < 13; i++)
  {
    ...
    // devices[i] = csma[i].Install (nodeAdjacencyList [i]);
  }
  ...
  // tch.AddInternalQueues (handle, 3, "ns3::DropTailQueue<Packet>", "MaxSize", StringValue ("1p"));
  // tch.AddInternalQueues (handle, 1, "ns3::DropTailQueue<Packet>", "MaxSize", StringValue ("1p"));
  // tch.AddInternalQueues (handle, 1, "ns3::DropTailQueue<Packet>", "MaxSize", StringValue ("1p"));
  // std::vector<QueueDiscContainer> qdisc(13);
  // for (int i = 0; i < 13; i ++)
  // {
  ...
  for (int i=0; i < 10; i ++)
  {
    ...
    std::ostringstream subset;
    subset<<i+1<<"Mbps";
    InstallBEPacketSend (nodes.Get (0), sinkAddress, startTime, endTime, Packetsize, NPacket, subset.str().c_str(), true);
    // InstallDGPacketSend (nodes.Get (0), sinkAddress, startTime, endTime, Packetsize, NPacket, budget, subset.str().c_str(), true);
  }



  // ---------------- Net Anim ---------------------
  AnimationInterface anim(dir + "exp1-NetAnim.xml");
