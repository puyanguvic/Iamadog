...
// void
// DropAtQueue (Ptr<QueueDiscClass> queueDiscClass, std::string qID)
// {
//   std::ofstream fDropQueue (std::stringstream (dir + "/queueTraces/queue-size-" + qID + ".dat").str ().c_str (), std::ios::out | std::ios::app);
//   fDropQueue << Simulator::Now ().GetSeconds () << " 1 " << std::endl;
//   fDropQueue.close ();
// }
...
main (int argc, char *argv[])
{
  ...
  uint32_t budget = 25;
  InstallDGPacketSend (nodes.Get (2), sinkAddress, 0.5, 2.0, Packetsize, NPacket, budget, "2Mbps", true);
...
// ------------------------ Network DSR TCP application--------------------------------------------
  ...
  // ---------------- Net Anim ---------------------
  AnimationInterface anim(dir + "dsr-experiment1.xml");
  ...
  // std::string qDrop = "n1n2-0";
  // Simulator::ScheduleNow (&CheckQueueSize, qdiscs2.Get (0), qDrop);
  ...
  streamWrapper = asciiTraceHelper.CreateFileStream (dir + "/queueTraces/drop-n1n4-0.dat");
  ...
  streamWrapper = asciiTraceHelper.CreateFileStream (dir + "/queueTraces/drop-n4n7-0.dat");
  ...
  // p2p.EnablePcapAll ("dsr-experiment1");
  // --------------- Store Recorded .dat --------------------
