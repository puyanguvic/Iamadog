...
{
  uint32_t qSize_fast = queueDisc-> GetInternalQueue (0)-> GetCurrentSize ().GetValue ();
  uint32_t qSize_slow = queueDisc-> GetInternalQueue (1)-> GetCurrentSize ().GetValue ();
  uint32_t qSize_norm = queueDisc-> GetInternalQueue (2)-> GetCurrentSize ().GetValue ();
  uint32_t qMax_fast = queueDisc-> GetInternalQueue (0)-> GetMaxSize ().GetValue ();
  uint32_t qMax_slow = queueDisc-> GetInternalQueue (1)-> GetMaxSize ().GetValue ();
  uint32_t qMax_norm = queueDisc-> GetInternalQueue (2)-> GetMaxSize ().GetValue ();
  // Check queue size every 1/100 of a second
  Simulator::Schedule (Seconds (0.001), &CheckQueueSize, queueDisc, qID);
  std::ofstream fPlotQueue (std::stringstream (dir + "/queue-size-" + qID + ".dat").str ().c_str (), std::ios::out | std::ios::app);
  fPlotQueue << Simulator::Now ().GetSeconds () << " " << qSize_fast << " " << qSize_slow << " " << qSize_norm << std::endl;
  fPlotQueue << Simulator::Now ().GetSeconds () << " " << qMax_fast << " " << qMax_slow << " " << qMax_norm << std::endl;
  fPlotQueue.close ();
}
...
main (int argc, char *argv[])
{
  ...
  std::string qID = "n2n1";
  Simulator::ScheduleNow (&CheckQueueSize, qdisc[7].Get (1), qID);
  ...
  for (int i=1; i<11; i++)
  {
    InstallDGPacketSend (nodes.Get(2), sinkAddress, (i-1)*0.5, i*0.5, PacketSize, NPacket, budget, 10, true);
  }
...
// ------------------------ Network DSR TCP application--------------------------------------------
  ...
  uint32_t budget1 = 20;
  uint32_t budget2 = 20;
  // InstallDGPacketSend (nodes.Get(2), sinkAddress_1, BeginTime, StopTime, PacketSize, NPacket1, budget1, 1, false);
  // InstallDGPacketSend (nodes.Get(0), sinkAddress_2, BeginTime, StopTime, PacketSize, NPacket1, budget2, 1, false);
  for (int i=1; i<11; i++)
  {
    InstallDGPacketSend (nodes.Get(2), sinkAddress_1, (i-1)*0.5, i*0.5, PacketSize, NPacket1, budget1, 2+i*0.5, false);
    InstallDGPacketSend (nodes.Get(0), sinkAddress_2, (i-1)*0.5, i*0.5, PacketSize, NPacket1, budget2, 2+i*0.5, false);
  }
